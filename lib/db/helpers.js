...
module.exports = {
  ...
  aggregateActiveClients: function aggregateActiveClients(activeClientTokens) {
    ...
    activeClientTokens.forEach(function (clientTokenObj) {
      ...
      // XXX TODO: this restores quadratic merge behavior! :-(
      activeClients[clientIdHex].scope = activeClients[clientIdHex].scope.add(clientTokenObj.scope);
    ...
    var activeClientsArray = Object.keys(activeClients).map(function (key) {
      activeClients[key].scope = activeClients[key].scope.toJSON().sort();
