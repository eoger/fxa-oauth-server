...
const UNTRUSTED_CLIENT_ALLOWED_SCOPES = Scope.fromArray([
  'openid',
  'profile:uid',
  'profile:email',
...
module.exports = {
  ...
  handler: function authorizationEndpoint(req, reply) {
    ...
    var exitEarly = false;
    var scope = Scope.fromString(req.payload.scope || '');
    P.all([
      verify(req.payload.assertion).then(function(claims) {
        ...
        if (! claims['fxa-tokenVerified']) {
          return P.each(scope.toJSON(), scope => {
      ...
      db.getClient(Buffer.from(req.payload.client_id, 'hex')).then(function(client) {
        ...
        if (! client) {
          ...
          var invalidScopes = scope.subtract(UNTRUSTED_CLIENT_ALLOWED_SCOPES);
          if (! invalidScopes.isEmpty()) {
            throw AppError.invalidScopes(invalidScopes.toJSON());
          }
        }
