...
module.exports = {
  ...
  handler: function keyDataRoute(req, reply) {
    ...
    const requestedScopes = Scope.fromString(req.payload.scope);
    ...
    P.all([
      ...
      db.getClient(Buffer.from(requestedClientId, 'hex')).then((client) => {
        if (client) {
          ...
          const allowedScopes = Scope.fromString(client.allowedScopes);
          const scopeLookups = requestedScopes.filter(allowedScopes).toJSON().map(s => db.getScope(s));
