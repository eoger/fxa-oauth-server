...
const SCOPE_OPENID = Scope.fromArray(['openid']);
...
module.exports = {
  ...
  handler: function tokenEndpoint(req, reply) {
    ...
    params.scope = Scope.fromString(params.scope || '');
    P.try(function() {
...
function confirmJwt(params) {
  ...
  return SERVICE_JWTOOL.verify(assertion).catch(function(err) {
    ...
    const requestedScope = Scope.fromString(payload.scope);
    if (! Scope.fromString(client.scope).contains(requestedScope)) {
